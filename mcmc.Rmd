...
Consider a sequence of random variables $X_0, X_1, X_2, \ldots$ where $X_n$ denotes the **state** at time $n \geq 0$. If the sequence is a [Markov chain](https://en.wikipedia.org/wiki/Markov_chain) then the the probability of $X_n$ depends only on $X_{n-1}$. Thus, we can write the conditional probability as,
$$
P(X_{n} = j | X_{n-1} = i)
$$
...
\vec{\mu}_1 \sim \mathcal{N}(0, 3) \\
\vec{\mu}_2 \sim \mathcal{N}(0, 3) \\
$$
...
The [Hamiltonian Monte Carlo](https://en.wikipedia.org/wiki/Hybrid_Monte_Carlo) algorithm is a concept borrowed from Physics that does a more efficient job of exploring the target distribution. It does this by suppressing the random walk behavior in Metropolis-Hastings. First, we need to introduce some concepts.
...
2. Repeat a-c $L$ times to complete one leapfrog step.
    ...
    b. Update the parameter vector $\vec{\theta} \leftarrow \vec{\theta} + \varepsilon M^{-1}\vec{\phi}$
    c. Complete the update of the momentum vector$\phi \leftarrow \phi + \frac{1}{2}\varepsilon \triangledown f(\vec{\theta})$.
...
We are modeling the data as being generated from the bivariate normal distribution with known covariance matrix $\Sigma$ and standard normal priors on the mean parameters. Formally,
$$
