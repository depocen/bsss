 ...
 * **Kernel** - the part of the probability function that exclusively depends on the random variable.
...
For example let $x ~ Beta(\alpha,\beta)$. The functional form is as follows,
$$
...
* The joint distribution is equal to the product of the marginal distributions: $f_{xy}(x,y) = f_{x}(x)\cdot f_{y}(y)$
...
$$
This can be thought of as weighting the random variable by its corresponding mass (or frequency of occurance). The mean (in the traditional sense) is a special case of the expected value where the weights associated with each value are all equal. 
...
$$
\mbox{continuous random variable}\ \implies E[x] = \int_{x\in X} g(x) \cdot f(x)\ dx \\
\mbox{discrete random variable}\ \implies E[x] = \sum_x g(x) \cdot f(x)
$$
...
$$
 ...
 What this is saying is that the distribution of the unknown parameter of interest $\theta$ given the data $y$ is equal to the product of the likelihood of the data $p(y|\theta)$ and the prior distribution of the parameter $p(\theta)$ divided by the marginal distribution of the data $p(y)$. \\
...
$$
\int_{\Omega}p(y|\theta)p(\theta)d\theta
$$
...
Note that $p(y|\theta)$ is a function of $\theta$. By solving for $\theta$ we can find the parameter value that maximizes the likelihood of obtaining the data (i.e. the maximum likelihood estimate of $\theta$). As mentioned before we often work with the log of the likelihood function since it is more tractable. 
